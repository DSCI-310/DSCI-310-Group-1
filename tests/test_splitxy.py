...
class Test_splitxy:
    ...
    def test_desiredfeatures_string(self):
        ...
        print("testing whether passing in a list of desiredfeatures produces correct output" )
        X_train, _= splitxy.split_xy(train_df, "romantic", "G3")
        X_test, _ = splitxy.split_xy(test_df, "G2", "G3")
    ...
    def test_target_list(self):
        ...
        _, y_train= splitxy.split_xy(train_df, "Mjob", samplefeatures)
        _, y_test = splitxy.split_xy(test_df, "Mjob", samplefeatures)
    ...
    def test_target_string(self):
        ...
        print("testing whether passing in a single produces correct output")
        _, y_train = splitxy.split_xy(train_df, desiredfeatures, "G3")
        _, y_test = splitxy.split_xy(test_df, desiredfeatures, "G3")
    ...
    def test_faultyvariable_in_list(self):
        ...
        with pytest.raises(KeyError):
            X_train, y_train = splitxy.split_xy(train_df, ["G2", "asodfiajsdofi"], "G3")
            X_train, y_train = splitxy.split_xy(train_df, "G3", ["G2", "asodfiajsdofi"])
    ...
    def test_faulty_single_variable(self):
        ...
        with pytest.raises(KeyError):
            X_train, y_train = splitxy.split_xy(train_df, desiredfeatures, "There_will_be_nothing_here")
            X_train, y_train = splitxy.split_xy(train_df, "There_will_be_nothing_here", desiredfeatures)
    ...
    def test_incorrect_inputtype(self):
        ...
        with pytest.raises(TypeError):
            X_train, y_train = splitxy.split_xy(3, desiredfeatures, "G3")
            X_train, y_train = splitxy.split_xy(train_df, np.array([1, 2, 3, 4]), "G3")
            X_train, y_train = splitxy.split_xy(train_df, "G3", np.array([1, 2, 3, 4]))
    
    def test_common_features(self):
        print("make sure we dont have warning if we dont have a repeat feature")
        X_train2, y_train= splitxy.split_xy(train_df, "G3", "G2")
